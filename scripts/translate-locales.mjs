...
export function getPluralCategories(loc) {
  try {
    ...
    const categories = new Intl.PluralRules(loc).resolvedOptions().pluralCategories ?? ['one', 'other'];
    const canonicalOrder = ['zero', 'one', 'two', 'few', 'many', 'other'];
    return canonicalOrder.filter(category => categories.includes(category));
  } catch {
    return ['one', 'other'];
  }
}
