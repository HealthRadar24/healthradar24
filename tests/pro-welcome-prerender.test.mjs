...
import { applyHealthRadarTextBrand } from '../pro-test/healthradar-brand.mjs';
...
test('welcome FAQPage JSON-LD matches every visible FAQ entry', () => {
  ...
  for (let n = 1; n <= WELCOME_FAQ_COUNT; n += 1) {
    ...
    assert.equal(entry.name, applyHealthRadarTextBrand(en.welcome.faq[`q${n}`]));
    assert.equal(entry.acceptedAnswer?.text, applyHealthRadarTextBrand(en.welcome.faq[`a${n}`]));
  }
});
