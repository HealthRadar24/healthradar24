...
(globalThis as unknown as { location: Location }).location = {
  ...
  hostname: 'healthradar24.com',
  protocol: 'https:',
  host: 'healthradar24.com',
} as Location;
...
describe('wm-session refresh-on-401 (Layer 2)', () => {
  it('retries an API 401 with a freshly-minted token', async () => {
    ...
    const resp = await wrappedFetch('https://api.healthradar24.com/api/bootstrap');
  ...
  it('does NOT retry when the path is in PREMIUM_RPC_PATHS', async () => {
    ...
    const resp = await wrappedFetch('https://api.healthradar24.com/api/market/v1/analyze-stock');
