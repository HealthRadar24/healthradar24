...
describe('widget-agent unified tester key auth', () => {
  ...
  it('accepts X-WorldMonitor-Key and upgrades relay request to pro', async () => {
    const res = await handler(new Request('https://www.healthradar24.com/api/widget-agent', {
      method: 'POST',
      headers: {
    ...
    assert.equal(call.arguments[0], 'https://proxy.healthradar24.com/widget-agent');
