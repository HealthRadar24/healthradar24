...
// fetching live api.healthradar24.com from CI would false-positive during
...
//   - Origin echo broken (preflight echoes `https://healthradar24.com` for an
...
const ENDPOINTS = [
  'https://api.healthradar24.com/api/health',
  'https://api.healthradar24.com/api/bootstrap?tier=fast',
];
...
// OAuth/MCP), not the Worker's healthradar24.com-only echo.
const PUBLIC_CORS_PROBES = [
  { url: 'https://api.healthradar24.com/api/mcp', origin: 'https://claude.ai' },
  { url: 'https://api.healthradar24.com/api/oauth/register', origin: 'https://claude.com' },
  { url: 'https://api.healthradar24.com/api/oauth-protected-resource', origin: 'https://claude.ai' },
];
...
for (const { url, origin } of PUBLIC_CORS_PROBES) {
  test(`OPTIONS ${url} from ${origin} bypasses Worker → Vercel ACAO survives`, { skip: !SHOULD_RUN }, async () => {
    ...
    // Acceptance: the response must NOT echo the canonical healthradar24.com
...
for (const url of ENDPOINTS) {
  test(`OPTIONS ${url} returns ACAC: true for ${ORIGIN}`, { skip: !SHOULD_RUN }, async () => {
    ...
    assert.equal(
      ...
      ORIGIN,
      'ACAO must echo the request origin (NOT https://healthradar24.com fallback, NOT *)',
    );
    assert.equal(
