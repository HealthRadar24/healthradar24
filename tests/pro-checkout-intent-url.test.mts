...
describe('buildCheckoutReturnUrl', () => {
  it('appends checkout params to a clean current URL', () => {
    const returnUrl = buildCheckoutReturnUrl('https://healthradar24.com/pro', 'pro_monthly');
  ...
  it('overwrites stale checkout params when the user clicks a different tier', () => {
    ...
    const firstClick = buildCheckoutReturnUrl('https://healthradar24.com/pro', 'pro_monthly');
  ...
  it('includes referralCode + discountCode when provided', () => {
    const returnUrl = buildCheckoutReturnUrl('https://healthradar24.com/pro', 'pro_annual', {
      referralCode: 'abc',
      discountCode: 'SAVE20',
    });
    const url = new URL(returnUrl);
  ...
  it('preserves unrelated query params on the current URL (utm, etc.)', () => {
    const returnUrl = buildCheckoutReturnUrl(
      'https://healthradar24.com/pro?utm_source=email',
      'pro_monthly',
    );
    const url = new URL(returnUrl);
  ...
  it('preserves pathname and hash (e.g., returning to /pro#enterprise)', () => {
    const returnUrl = buildCheckoutReturnUrl(
      'https://healthradar24.com/pro#pricing',
      'pro_monthly',
    );
    const url = new URL(returnUrl);
