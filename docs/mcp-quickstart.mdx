...
Upgrade or generate a key at [healthradar24.com/pro](https://www.healthradar24.com/pro). The rest of this guide assumes Claude Desktop + OAuth (the simplest path); if you'd rather paste a `wm_…` key into a `curl` script, jump to [Server-side curl](#server-side-curl) at the bottom.
...
```bash
export WM_KEY="wm_0123456789abcdef0123456789abcdef01234567"  # API Starter+ key from healthradar24.com/settings
