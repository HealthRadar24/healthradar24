...
PRO subscribers get an API key from [healthradar24.com/pro](https://www.healthradar24.com/pro). User-issued keys are exactly `wm_` followed by 40 lowercase hex characters.
...
```bash
curl -s 'https://api.healthradar24.com/api/conflict/v1/list-conflict-events?limit=5' \
...
const res = await fetch(
  'https://api.healthradar24.com/api/market/v1/get-quotes?symbols=AAPL,TSLA',
...
r = httpx.get(
    "https://api.healthradar24.com/api/economic/v1/get-fred-series",
...
```bash
curl -s 'https://api.healthradar24.com/api/bootstrap' \
  -H 'X-WorldMonitor-Key: wm_0123456789abcdef0123456789abcdef01234567'
```
...
```
https://api.healthradar24.com/api/mcp
```
