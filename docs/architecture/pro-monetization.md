...
Products are Dodo `productId`s stored client-side in `pro-test/src/generated/tiers.json` and served at runtime from `https://api.healthradar24.com/api/product-catalog`:
...
- **Enterprise** — `mailto:enterprise@healthradar24.com` (contact sales).
...
1. Validates `returnUrl` against an allow-listed set of healthradar24.com origins.
...
- **Full-page return** (fallback / `/pro` path): Dodo redirects to `healthradar24.com/?subscription_id=...&status=active`. `src/services/checkout-return.ts:handleCheckoutReturn()` reads params, cleans the URL, returns success boolean.
...
- **Share link**: `https://healthradar24.com/pro?ref=<code>`.
