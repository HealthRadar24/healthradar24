...
description: "Design principles, intelligence tradecraft, algorithmic decisions, and implementation patterns behind HealthRadar24."
---

> **Looking for the system reference?** See [`ARCHITECTURE.md`](https://github.com/HealthRadar24/healthradar24/blob/main/ARCHITECTURE.md) for deployment topology, directory layout, caching layers, CI/CD, and contributor how-to guides.
...
HealthRadar24 is written in vanilla TypeScript — no frontend framework (React, Vue, Svelte, Angular) is used. This is a deliberate architectural decision, not an oversight.
...
All six dashboard variants (HealthRadar24, Tech Monitor, Finance Monitor, Commodity Monitor, Happy Monitor, Energy Monitor) serve from a **single Vercel deployment**. The variant is determined at runtime by hostname detection:
...
| `healthradar24.com` (default) | `full` |
...
Sentry captures unhandled exceptions and promise rejections in production, with environment-aware routing (production on `healthradar24.com`, preview on `*.vercel.app`, disabled on localhost and Tauri desktop).
