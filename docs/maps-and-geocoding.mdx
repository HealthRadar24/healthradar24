...
| URL | Use |
|-----|-----|
| `https://maps.healthradar24.com/<file>` | Production URL (CF-proxied, cached, CORS headers) |
...
2. `country-boundary-overrides.geojson` — optional higher-resolution polygons from [Natural Earth](https://www.naturalearthdata.com/), served from R2 CDN (`maps.healthradar24.com`). Features matched by `ISO3166-1-Alpha-2` (or `ISO_A2`) code; matching features replace the base geometry
...
The override mechanism lets us improve individual country boundaries without replacing the entire `countries.geojson`. This is the foundation for addressing disputed borders (see [#1044](https://github.com/HealthRadar24/healthradar24/issues/1044)).
...
PMTiles are also served from R2 via `maps.healthradar24.com`, configured through `VITE_PMTILES_URL`.

## Common Mistakes

| Mistake | Fix |
|---------|-----|
| Using `pub-*.r2.dev` URLs in code | Always use `maps.healthradar24.com` (CF-proxied) |
