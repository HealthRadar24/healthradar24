...
description: "Point any AI agent, codegen tool, or MCP client at healthradar24.com and discover every API, OpenAPI spec, MCP server, and OAuth endpoint from a single root URL."
---
...
No prior knowledge required. Just `GET https://healthradar24.com/`.
...
```bash
curl -sI https://healthradar24.com/ | grep -i '^link:'
```
...
|---|---|---|
| [`/.well-known/api-catalog`](https://healthradar24.com/.well-known/api-catalog) | [RFC 9727](https://datatracker.ietf.org/doc/html/rfc9727) | JSON linkset bundling every other discovery URL — start here if you want one fetch and a map |
| [`/openapi.yaml`](https://www.healthradar24.com/openapi.yaml) | OpenAPI 3.1 | Single bundled spec covering **every** REST service (Conflict, Resilience, Market, Economic, Maritime, Aviation, Climate, …) — feed it to any code-generator |
| [`/.well-known/mcp/server-card.json`](https://healthradar24.com/.well-known/mcp/server-card.json) | MCP | Transport (`streamableHttp`), endpoint, OAuth resource, scopes, capability flags |
| [`/.well-known/oauth-authorization-server`](https://api.healthradar24.com/.well-known/oauth-authorization-server) | [RFC 8414](https://datatracker.ietf.org/doc/html/rfc8414) | OAuth 2.1 authorization-server metadata (PKCE, DCR, token endpoints) |
| [`/.well-known/oauth-protected-resource`](https://healthradar24.com/.well-known/oauth-protected-resource) | [RFC 9728](https://datatracker.ietf.org/doc/html/rfc9728) | Resource metadata for `https://healthradar24.com/mcp` |
| [`/.well-known/agent-skills/index.json`](https://healthradar24.com/.well-known/agent-skills/index.json) | (custom) | Index of pre-packaged agent skills (`fetch-country-brief`, `fetch-resilience-score`, …) — each skill is a self-contained recipe |
| [`/llms.txt`](https://healthradar24.com/llms.txt) | [llmstxt.org](https://llmstxt.org) | LLM-friendly markdown briefing — overview, capabilities, links |
| [`/llms-full.txt`](https://healthradar24.com/llms-full.txt) | (extension) | Long-form variant covering all data layers, panels, and data sources |
| [`/api/health`](https://api.healthradar24.com/api/health) | (custom) | Per-key seed status, freshness, record counts — agents can gate fetches on this |
...
curl -s https://healthradar24.com/.well-known/api-catalog \
  | jq -r '.linkset[0]."service-desc"[0].href'
# → https://www.healthradar24.com/openapi.yaml

# 2. Generate clients
curl -s https://www.healthradar24.com/openapi.yaml -o worldmonitor.openapi.yaml
...
curl -s https://healthradar24.com/.well-known/mcp/server-card.json
# → endpoint (https://healthradar24.com/mcp), transport, OAuth scopes,
#   and authorization_servers: ["https://api.healthradar24.com"]

# 2. Fetch authorization-server metadata from that host
curl -s https://api.healthradar24.com/.well-known/oauth-authorization-server
...
```bash
curl -s https://api.healthradar24.com/api/resilience/v1/get-resilience-ranking \
  -H "X-WorldMonitor-Key: $WM_KEY"
```

PRO subscribers get a key from [healthradar24.com/pro](https://www.healthradar24.com/pro). See [Authentication](/usage-auth).
