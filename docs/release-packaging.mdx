...
description: "This guide provides reproducible local packaging steps for both desktop variants: full (HealthRadar24) and tech (Tech Monitor)."
---
...
- Full variant: `HealthRadar24` / `world-monitor`
- Tech variant: `Tech Monitor` / `tech-monitor`

Distinct names are configured in Tauri:

- `src-tauri/tauri.conf.json` → `HealthRadar24` / `world-monitor`
