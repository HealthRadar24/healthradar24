...
| Endpoint | Purpose |
|----------|---------|
| `https://healthradar24.com/mcp` | JSON-RPC server (Streamable HTTP transport; `initialize` defaults to protocol `2025-03-26`) |
| `https://api.healthradar24.com/api/oauth/register` | Dynamic Client Registration (RFC 7591) |
| `https://api.healthradar24.com/api/oauth/authorize` | OAuth 2.1 authorization endpoint (PKCE required) |
| `https://api.healthradar24.com/api/oauth/token` | Token endpoint (authorization_code + refresh_token) |
| `https://api.healthradar24.com/.well-known/oauth-authorization-server` | AS metadata (RFC 8414) |
| `https://healthradar24.com/.well-known/oauth-protected-resource` | Resource server metadata (RFC 9728) |
...
1. **Add the server URL** in your MCP client. The canonical entrypoint is:
   ...
   ```
   https://api.healthradar24.com/mcp
   ```

   (`https://healthradar24.com/mcp` works too — it proxies the same handler.)

2. **Click "Sign in with WorldMonitor Pro"** on the consent page. This is the default CTA. You'll bounce through `healthradar24.com/mcp-grant` (Clerk-protected — sign in if needed), then back to `api.healthradar24.com/oauth/authorize-pro`, then to your client's redirect.
...
- Manage connected clients in **[Settings → Connected MCP clients](https://healthradar24.com/settings)**.
...
|------|------------|------------|-----------------------|-------|
| **Free** | ❌ No | — | — | OAuth flow returns 401 `INSUFFICIENT_TIER`. Upgrade at [healthradar24.com/pro](https://www.healthradar24.com/pro). |
...
```json
{
  "mcpServers": {
    "worldmonitor": {
      "url": "https://healthradar24.com/mcp"
    }
  }
}
```
...
```bash
npx @modelcontextprotocol/inspector https://healthradar24.com/mcp
```
...
# 1. List tools
curl -s https://healthradar24.com/mcp \
...
Seed-level health per key: [`api.healthradar24.com/api/health`](https://api.healthradar24.com/api/health).
...
- [Pro features](https://www.healthradar24.com/pro)
