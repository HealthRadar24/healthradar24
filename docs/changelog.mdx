---
title: "Changelog"
description: "All notable changes to HealthRadar24, organized by release."
rss: true
---

All notable changes to HealthRadar24 are documented here. Subscribe via [RSS](/changelog/rss.xml) to stay updated.
...
- **Sebuf API migration (#3207)** — scenario + supply-chain endpoints moved to the typed sebuf contract. RPC URLs now derive from method names; five renamed v1 URLs remain live as thin aliases so existing integrations keep working:
  ...
  Aliases retire at the next v1→v2 break (tracked in [#3282](https://github.com/HealthRadar24/healthradar24/issues/3282)).
...
- **CDN cache bypass closed**: `CDN-Cache-Control` header now only emitted for trusted origins (healthradar24.com, Vercel previews, Tauri). No-origin server-side requests always invoke the edge function so `validateApiKey` runs, preventing a cached 200 from being served to external scrapers.
...
- **Edge caching**: Cloudflare edge caching for proxy.healthradar24.com (#478) and api.healthradar24.com (#471)
...
- **Finance variant**: Added a dedicated market-first variant (`finance.healthradar24.com`) with finance/trading-focused feeds, panels, and map defaults
