...
function publicCatalog(payload) {
  const data = withPublicFacts(payload);
  if (!PAYMENTS_ENABLED) {
    return {
      ...data,
  ...
  if (BILLING_PROVIDER !== 'stripe') {
    return { ...data, commerceEnabled: true, commerceProvider: 'dodo' };
  }
  return {
    ...data,
...
export default async function handler(req) {
  ...
  // The Railway seed contains Dodo product IDs. Never serve it as a Stripe
  // catalog; Stripe prices are resolved from fork-owned Price IDs below.
  ...
  if (PAYMENTS_ENABLED && BILLING_PROVIDER === 'stripe') {
    ...
    return json(publicCatalog(result), 200, cors, 'public, max-age=60, s-maxage=60', result.priceSource);
  }
