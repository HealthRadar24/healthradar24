...
function makeRequest(query = '') {
  return new Request(`https://healthradar24.com/api/youtube/embed${query}`);
}
...
test('returns embeddable html for valid video id', async () => {
  ...
  assert.equal(html.includes('origin:"https://healthradar24.com"'), true);
...
test('uses dedicated parentOrigin for iframe postMessage target', async () => {
  const response = await handler(makeRequest('?videoId=iEpJwprxDdk&origin=https://healthradar24.com&parentOrigin=https://tauri.localhost'));
  const html = await response.text();
  assert.match(html, /playerVars:\{[^}]*origin:"https:\/\/healthradar24\.com"/);
...
test('does not accept wildcard parentOrigin query parameter', async () => {
  const response = await handler(makeRequest('?videoId=iEpJwprxDdk&origin=https://healthradar24.com&parentOrigin=*'));
  const html = await response.text();
  assert.equal(html.includes('parentOrigin="*"'), false);
  assert.match(html, /parentOrigin="https:\/\/healthradar24\.com"/);
});
