...
function makePreflight(origin) {
  return new Request('https://api.healthradar24.com/api/health?compact=1', {
    method: 'OPTIONS',
    headers: {
      origin,
...
test('health GET response is compatible with credentialed browser fetches', async () => {
  const resp = await handler(new Request('https://api.healthradar24.com/api/health?compact=1', {
    method: 'GET',
    headers: {
