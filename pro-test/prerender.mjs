...
for (const { file, content, rootAttributes } of PAGES) {
  ...
  const rootPattern = /<div id="root"(?:\s[^>]*)?>[\s\S]*?<\/div>(?=\s*<noscript>)/;
  if (!rootPattern.test(html)) {
    console.error(`[prerender] ERROR: ${file} has no replaceable root container before <noscript>.`);
    process.exit(1);
  }
  html = html.replace(
    rootPattern,
    `<div id="root"${rootAttributes}>${applyHealthRadarHtmlBrand(content)}</div>`,
  );
  html = applyHealthRadarHtmlBrand(html);
  html = applyHealthRadarTextBrand(html);
  writeFileSync(htmlPath, html, 'utf-8');
  console.log(`[prerender] Injected HealthRadar critical CSS and visible content into public/pro/${file}`);
}
