...
const CATALOG_API = 'https://api.healthradar24.com/api/product-catalog';
...
function isInProductHref(href: string): boolean {
  ...
  try {
    const base = typeof window !== 'undefined' ? window.location.href : 'https://healthradar24.com/';
    const url = new URL(href, looksRelative ? base : undefined);
    if (url.protocol !== 'http:' && url.protocol !== 'https:') return false;
    return url.hostname === 'healthradar24.com' ||
...
function getCtaProps(tier: Tier, billing: 'monthly' | 'annual'): CtaProps {
  if (tier.cta && tier.href && tier.price === 0) {
    // Free tier on an in-product href (e.g. healthradar24.com dashboard)
